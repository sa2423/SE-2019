...
public class FacePermissionAcitivity extends Activity {
	...
	public void startActiviy() {
		if (mRequestPermission.isEmpty()) {
			...
			new Thread(new Runnable() {
				@Override
				public void run() {
					FaceApplication app = (FaceApplication) FacePermissionAcitivity.this.getApplicationContext();
					app.mFaceDB.loadFaces();
					FacePermissionAcitivity.this.runOnUiThread(new Runnable() {
						@Override
						public void run() {
							mProgressDialog.cancel();
							Intent intent = new Intent(FacePermissionAcitivity.this, FaceMainActivity.class);
			...
			new Handler().postDelayed(new Runnable() {
				@Override
				public void run() {
					FacePermissionAcitivity.this.finish();
				}
			}, 3000);
		}
	}
